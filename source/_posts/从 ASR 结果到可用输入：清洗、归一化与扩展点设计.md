...
> 本文是 [事件驱动的 ASR → LLM → ES 回写：设计拆解与实践](事件驱动的%20ASR%20→%20LLM%20→%20ES%20回写：设计拆解与实践.md) 系列的第一篇，重点讲解 ASR 结果的清洗与归一化设计。
...
下一篇继续看：整理好的结果是怎样进入分析链路的，以及多个阶段如何按顺序执行。详见 [把 LLM 放进分析链路：阶段编排与事件触发](把%20LLM%20放进分析链路：阶段编排与事件触发.md)。
