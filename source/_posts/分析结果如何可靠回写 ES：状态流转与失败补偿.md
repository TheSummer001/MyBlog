...
> 本文是 [事件驱动的 ASR → LLM → ES 回写：设计拆解与实践](事件驱动的%20ASR%20→%20LLM%20→%20ES%20回写：设计拆解与实践.md) 系列的第三篇，重点讲解 ES 回写的状态流转与失败补偿设计。
...
到这里，ASR、LLM 和 ES 的整条链路就基本串起来了。可以回到 [概述文章](事件驱动的%20ASR%20→%20LLM%20→%20ES%20回写：设计拆解与实践.md) 回顾整体设计思路。
