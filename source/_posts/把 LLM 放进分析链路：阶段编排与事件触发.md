...
> 本文是 [事件驱动的 ASR → LLM → ES 回写：设计拆解与实践](事件驱动的%20ASR%20→%20LLM%20→%20ES%20回写：设计拆解与实践.md) 系列的第二篇，重点讲解 LLM 分析的阶段编排与事件驱动设计。
...
下一篇就可以更自然地理解：结果写回 ES 为什么要单独设计，状态流转为什么要和补偿放在一起。详见 [分析结果如何可靠回写 ES：状态流转与失败补偿](分析结果如何可靠回写%20ES：状态流转与失败补偿.md)。
